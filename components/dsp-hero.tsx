  ...
  return (
    <section className="relative min-h-screen flex items-center py-20 lg:py-0 overflow-hidden bg-background">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        {/* Animated Waveform Background Placeholder */}
      ...
      <div className="container relative z-10 mx-auto px-6">
        <div className="max-w-4xl">
          ...
          >
            ...
            <p className="text-base md:text-lg text-muted-foreground/80 local-inter leading-relaxed max-w-3xl">
              An automated signal processing-based system developed to identify spectral characteristics of embedded noise and apply targeted filtering algorithms to suppress interference while preserving musical integrity.
            </p>

            <div className="flex flex-wrap gap-4 pt-4">
              <Link
                href="/introduction"
                className="group relative inline-flex h-14 items-center justify-center overflow-hidden rounded-full bg-foreground px-10 text-sm font-bold text-background transition-all hover:scale-105"
              >
              ...
              <a
                ...
                className="inline-flex h-14 items-center justify-center rounded-full border border-border bg-card/40 px-10 text-sm font-bold backdrop-blur-xl transition-all hover:bg-muted uppercase tracking-widest flex items-center gap-2"
              >
                <FileText size={18} />
                View Full Report
              </a>
          ...
          >
            {[
              ...
              { label: "Sample Rate", value: "44.1 kHz", icon: Zap },
              { label: "Performance Metrics", value: "4", icon: Database },
            ].map((metric, idx) => (
              <div key={idx} className="p-6 rounded-2xl border border-border bg-card/50 backdrop-blur-sm group hover:border-primary/50 transition-colors">
                <metric.icon className="w-5 h-5 text-primary mb-4 group-hover:scale-110 transition-transform" />
                <div className="text-2xl font-bold font-google-sans mb-1">{metric.value}</div>
                <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-black">{metric.label}</div>
              </div>
            ))}
          </motion.div>
        </div>
